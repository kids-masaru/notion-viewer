...
// Component to display relation names
function RelationDisplay({ relations, apiKey }: { relations: any[]; apiKey: string }) {
    ...
    if (loading) {
        return (
            <div className="flex items-center gap-2">
                ...
                <span className="text-gray-500 text-sm">Loading {relations.length} relation(s)...</span>
            </div>
        );
    }
        ...
        <div className="flex items-start gap-2">
            ...
            <div className="flex flex-wrap gap-1">
                {relations.map((rel) => (
                    <span
                        key={rel.id}
                        className="inline-block px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-sm"
                    >
